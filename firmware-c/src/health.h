 ...
 * spinlock and renders. The device does NO HP math — `lit` arrives precomputed
 * from the server (0..NUM_LEDS).
 */
...
typedef struct {
    anim_state_t state;
    int          lit;         // 0..NUM_LEDS, server-precomputed
    int          age_s;       // upstream staleness from line 1
    uint32_t     updated_ms;  // when core0 last set lit (for flash detection)
} health_t;

/* core0 API (thread-safe via spinlock). */
void     health_init(void);
void     health_set_state(anim_state_t st);
void     health_set_lit(int lit, int age_s);  // also sets ST_LIVE + updated_ms
